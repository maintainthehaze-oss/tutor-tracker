...
**2026-09-15 (Fable): TRASH CAN ON LOCKED ROWS NOW EXPLAINS + OFFERS CANCEL. New sessions default to Completed and lock on save, and the disabled trash button looked clickable but did nothing. Now: locked row trash -> confirm -> append-only status correction to cancelled (paid rows refused until un-paid; already-cancelled rows just say so). Disabled buttons dimmed via CSS. Deletion of locked rows stays impossible by policy. COMMITTED, NOT PUSHED (awaiting "Ship it"). Detail: `docs/handoffs/2026-09-15-mileage-autocalc.md` (follow-up 3).**
