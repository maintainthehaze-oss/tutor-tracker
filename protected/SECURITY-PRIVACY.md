...
## Isolated preview override - 2026-09-06, Slice 4

The audit below describes the original app, not this synthetic preview. Current
preview records use the dedicated tutor-tracker-slice1-preview IndexedDB repository.
The protected active store is schema 2; connection metadata and a local rehearsal
mirror use separate keys in that preview database. No legacy localStorage is read.

The preview has no live Gist transport, API credentials form or auto-sync. User-run
portable backup downloads contain complete fabricated archive/report/receipt data;
matching backups can be staged and applied with archive protections. Recognized
nonempty credential/connection fields block portable export/import, including raw
JSON evidence. They are not silently redacted. This does not detect arbitrary
secrets embedded in free text. Connection metadata is not part of portable snapshots.

Sync rehearsal is an additional local browser copy, with conditional writes and
stable identity recovery. It sends no records to GitHub. This does not change the
original historical-upload policy. Live upload/privacy changes remain owner decisions.
Reporting libraries are fixed local distributions. CSP remains same-origin for
scripts/connections. Default service-worker registration stays disabled; sw.js is
loopback-only rehearsal code and never claims a production upgrade.

See SLICE4_NOTES.md and ../docs/SLICE4_TRANSITION.md. Original app, live data and the
prior audit evidence below are unchanged. No deployment has occurred.

## Original application audit (retained)
...
2. **OpenRouteService** (`js/sessions.js`, mileage auto-calc) → only when the
   ...
   key. The key travels in the Authorization header for both calls (never the
   URL). The key is stored in local settings only and is excluded from the
   sync allowlist above. No bulk or background recalculation exists.
