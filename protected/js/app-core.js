...
(function () {
  ...
  /** Application state */
  let clients = Object.freeze([]);
  let sessions = Object.freeze([]);
  let expenses = Object.freeze([]);
  let settings = Object.freeze({ ...DEFAULT_SETTINGS });
  let receipts = Object.freeze({});
  let taxPayments = Object.freeze([]);
  ...
  App.state = {
    ...
    get yoyChart() { return yoyChart; },
    set yoyChart(v) { yoyChart = v; },
  ...
  Object.freeze(App.state);

  // The repository owns data. These locals are frozen read views only.
  function refreshReadViews() {
    const view = App.repository.read();
    clients = view.clients; sessions = view.sessions; expenses = view.expenses;
    settings = view.settings; receipts = view.receipts; taxPayments = view.taxPayments;
  }
  async function loadData() {
    const ready = await App.repository.open();
    refreshReadViews();
    return ready;
  }
  App.refreshReadViews = refreshReadViews;
  App.isProtectedSession = id => !App.repository.ready || App.repository.isProtectedSession(id);
  App.isArchivedRecord = (collection,id) => !App.repository.ready || App.repository.isArchivedRecord(collection,id);
  App.runCommand = async (name,payload,revision) => {
    try { await App.repository.execute(name,payload,revision); }
    catch(error) { App.showToast(error.message,'error'); return false; }
    try {
      refreshReadViews(); App.renderTab(activeTab); App.updateHeaderStats(); App.updateProtectionStatus();
      const el=$('last-saved'); if(el)el.textContent='Saved protected revision '+App.repository.revision;
    } catch(error) { App.showToast('Saved successfully, but the screen could not refresh. Reload to view the saved record.','warning'); }
    return true;
  };
  ...
  function initTheme() {
    const saved = null; // Preview theme is tab-local; no legacy storage access.
    if (saved) {
