...
(function () {
  ...
  let expenseFormRevision;

  function openExpenseForm(id) {
    expenseFormRevision = App.repository.revision;
    if (id && App.isArchivedRecord('expenses', id)) {
      App.showToast('Original expenses and receipts are read-only', 'info');
      return;
    }
    const expenses = App.state.expenses;
    const receipts = App.state.receipts;
  ...
  async function saveExpense() {
    const revision = expenseFormRevision;
    ...
    if (isNew) expenseData.createdAt = new Date().toISOString();
    if (await App.runCommand('expense.save', {
      record: expenseData,
  ...
  function deleteExpense(id) {
    ...
    const e = App.state.expenses.find((ex) => String(ex.id) === String(id));
    if (!e) return;
    if (App.isArchivedRecord('expenses', id)) {
      App.showToast('Original expenses and receipts are read-only', 'info');
      return;
    }
    App.showConfirm('Delete Expense', 'Delete this expense from ' + formatDate(e.date) + '?', async () => {
      if (await App.runCommand('expense.delete', { id }, revision)) {
