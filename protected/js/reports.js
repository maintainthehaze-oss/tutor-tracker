...
(function () {
  ...
  function populateFamilyFilter() {
    ...
    if (!sel) return;
    const clients = App.reportModel.clients(App.reportModel.mode('report'));
  ...
  function renderReports() {
    ...
    const M = App.reportModel.period(filter, mode);
    ...
    const totalExpenses = M.totalExpenses;
    const mileageDed = M.mileageDeduction;
    ...
    if (tbody) {
      ...
      for (let m = 0; m < 12; m++) {
        ...
        const mm = App.reportModel.period(mf, mode);
        const mExp = mm.totalExpenses;
        ...
        rows +=
          '<tr>' +
            ...
            '<td>' + formatCurrency(mm.mileageDeduction) + '</td>' +
          '</tr>';
      }
  ...
  function renderClientStats(year, filter) {
    const mode = App.reportModel.mode('report');
    const numeric = mode === 'captured-v1' ? num : App.reportModel.numeric;
    const clients = App.reportModel.clients(mode);
    ...
    const M = App.reportModel.metrics(filter || { year: String(year) }, mode);
  ...
  function sessionWorkState(s, sourceClients, warnings) {
    if (s.type !== 'in-person') {
      if (warnings && !['online', 'hybrid'].includes(s.type)) warnings.push('Record ' + s.id + ': unknown work type; legacy CT-source assumption retained.');
      return 'CT';
    }
    let addr = typeof s.address === 'string' ? s.address.trim() : '';
    if (!addr) {
      const clients = sourceClients || [];
      for (const cid of (s.clientIds || [])) {
        const c = clients.find((cl) => String(cl.id) === String(cid));
        if (c && typeof c.address === 'string' && c.address.trim()) { addr = c.address.trim(); break; }
      }
    }
    const st = addrState(addr);
    if (!st && warnings) warnings.push('Record ' + s.id + ': unknown work state; legacy CT-source assumption retained.');
    return st || 'CT'; // unknown address — assume home state
  }

  function datedInYear(date, year, mode) {
    return typeof date === 'string' && (mode === 'captured-v1' || validReportDate(date)) && date.slice(0, 4) === String(year);
  }
  function validReportDate(date) {
    return App.reportModel.validDate(date);
  }

  function getTaxData(year, mode = App.reportModel.mode('tax')) {
    const model = App.reportModel;
    const { sessions, expenses, clients, taxPayments } = model.context(mode);
    const num = mode === 'captured-v1' ? App.num : model.numeric;
    ...
    const cashSessions = sessions.filter((s) =>
      ...
      datedInYear(s.paymentDate || s.date, yearStr, mode)
    );
    ...
    const yearSessions = sessions.filter((s) =>
      datedInYear(s.date, yearStr, mode) && s.status === 'completed'
    );
    const yearExpenses = expenses.filter((e) => datedInYear(e.date, yearStr, mode));
    ...
    const mileageDeduction = yearSessions.reduce((sum, s) => sum + num(s.mileage) * num(model.rate(s, year, mode)), 0);
    const rates = [...new Set(yearSessions.map(s => model.rate(s, year, mode)))];
    const mileageRate = rates.length === 1 && rates[0] != null ? rates[0] : 0;
    const mileageRateLabel = rates.length === 1 && rates[0] != null ? '$' + mileageRate.toFixed(3) + '/mi (' + year + ')' : 'Per-session captured rates; see data notice';
    const relevantYears = new Set([yearStr, ...cashSessions.map(s => String(s.date || '').slice(0, 4))]);
    const warnings = [...new Set([...relevantYears].flatMap(y => model.metrics({ year: y }, mode).warnings || []))];
    if (mode !== 'captured-v1') {
      sessions.filter(s => s.status === 'completed').forEach(s => {
        if (!validReportDate(s.date)) warnings.push('Record ' + s.id + ': invalid or missing session date; excluded from dated mileage totals.');
        if (s.paid === true && !validReportDate(s.paymentDate || s.date)) warnings.push('Record ' + s.id + ': invalid or missing receipt date; excluded from dated cash totals.');
        if (datedInYear(s.date, yearStr) && typeof s.paid !== 'boolean') warnings.push('Record ' + s.id + ': payment status is unknown; legacy uncollected assumption retained.');
      });
      cashSessions.filter(s => !s.paymentDate).forEach(s => warnings.push('Record ' + s.id + ': payment date is missing; legacy session-date fallback retained.'));
      expenses.filter(e => !validReportDate(e.date)).forEach(e => warnings.push('Expense ' + e.id + ': invalid or missing date; excluded from dated expense totals.'));
      taxPayments.filter(p => !validReportDate(p.date)).forEach(p => warnings.push('Tax payment ' + p.id + ': invalid or missing date; excluded from dated payment totals.'));
      yearExpenses.forEach(e => { if (!model.validNumber(e.amount)) warnings.push('Unknown expense amount: ' + e.id); });
      yearSessions.forEach(s => { if (model.rate(s, year, mode) == null) warnings.push('Unknown mileage rate: ' + s.id); });
      taxPayments.filter(p => datedInYear(p.date, yearStr)).forEach(p => { if (!model.validNumber(p.amount)) warnings.push('Unknown estimated tax payment amount: ' + p.id); });
    }
    ...
    const sourceWarnings = mode === 'captured-v1' ? null : warnings;
    const nyCash = cashSessions.filter((s) => sessionWorkState(s, s._report ? s._report.clients : clients, sourceWarnings) === 'NY');
    const nyYear = yearSessions.filter((s) => sessionWorkState(s, s._report ? s._report.clients : clients, sourceWarnings) === 'NY');
    ...
    return {
      ...
      otherExpenses, expByCategory, mode, warnings: [...new Set(warnings)], mileageRateLabel,
    };
  }
  ...
  function renderTaxSummary() {
    ...
    const trEl = $('tax-irs-rate'); if (trEl) trEl.textContent = data.mileageRateLabel;
  ...
  function exportTaxPDF() {
    if (typeof window.jspdf?.jsPDF !== 'function' || typeof window.jspdf.jsPDF.API?.autoTable !== 'function') {
      App.showToast('PDF export unavailable in this preview. Use CSV.', 'error');
      return;
    }
    ...
    doc.autoTable({
      ...
      body: [
        ...
        ['10', 'Commissions and fees (historical share)', formatCurrency(data.line10)],
    ...
    doc.text('Mileage rates: ' + data.mileageRateLabel, 14, afterExp + 26);
    doc.text('Deduction: ' + formatCurrency(data.mileageDeduction), 14, afterExp + 32);

    doc.addPage();
    doc.setFontSize(10);
    doc.autoTable({
      startY: 18, margin: { bottom: 20 }, head: [['Report data notice']],
      body: [[App.reportModel.notice([], data.mode)],
        ...(data.warnings.length ? [['Incomplete inputs: amounts shown are known subtotals, not complete totals.'], ...data.warnings.map(w => [w])] : [])],
      theme: 'grid', headStyles: { fillColor: [108, 99, 255] },
    });
    const pages = doc.getNumberOfPages();
    for (let page = 1; page <= pages; page++) {
      doc.setPage(page);
      doc.setFontSize(8);
      doc.text('Tax year ' + year + ' | ' + data.mode + ' | Page ' + page + ' of ' + pages,
        14, doc.internal.pageSize.getHeight() - 10);
    }
    doc.save('TaxSummary_' + year + '_' + data.mode + '.pdf');
  ...
  function renderTaxPayments(year) {
    ...
    if (!tbody) return;
    const payments = (App.reportModel.context(App.reportModel.mode('tax')).taxPayments || [])
      .filter((p) => datedInYear(p.date, year, App.reportModel.mode('tax')))
    ...
    const number = App.reportModel.mode('tax') === 'captured-v1' ? num : App.reportModel.numeric;
  ...
  function openTaxPaymentForm(id) {
    taxFormRevision = App.repository.revision;
    if (id && App.isArchivedRecord('taxPayments', id)) {
      App.showToast('Original tax payments are read-only', 'info');
      return;
    }
  ...
  async function saveTaxPayment() {
    const revision = taxFormRevision;
    ...
    if (await App.runCommand('tax.save', { record: data }, revision)) {
  ...
  function deleteTaxPayment(id) {
    ...
    if (App.isArchivedRecord('taxPayments', id)) {
      App.showToast('Original tax payments are read-only', 'info');
      return;
    }
    App.showConfirm('Delete Payment', 'Remove this estimated tax payment from your records?', async () => {
      if (await App.runCommand('tax.delete', { id }, revision)) {
  ...
  App.getReportFilter = getReportFilter;
