/* Loaded before App. Offline caching and the install-as-app behaviour were removed
   ...
   1. Unregisters any tracker service worker (protected scope AND retired root scope)
      still installed in this browser and deletes their caches, so every load is served fresh from GitHub Pages.
...
(function () {
  ...
  async function removeOfflineWorker() {
    ...
    try {
      ...
      // Both the protected scope and the retired root scope (root worker killed 2026-09-10).
      await Promise.all(regs
        .filter(reg => reg.scope.includes('/tutor-tracker/') || !production)
        .map(reg => reg.unregister()));
      if (window.caches) {
        const keys = await caches.keys();
        await Promise.all(keys.filter(k => k.startsWith('tutor-')).map(k => caches.delete(k)));
      }
